...
public class ColoredRedstone extends Block {
    ...
    public ColoredRedstone(AbstractBlock.Settings settings) {
        super(settings);
    ...
    protected static boolean connectsTo(BlockState state, @Nullable Direction dir) {
        if (state.isOf(Blocks.REDSTONE_WIRE)) {
            return false;
        } else if (state.isOf(FluxTechBlocks.BLUESTONE)) {
            return true;
...
COLORS = new int[16];
        for(int i = 0; i <= 15; ++i) {
            int blue = (int) MathHelper.lerp(i / 15f, 0f, 255f);
            COLORS[i] = blue;
        }
