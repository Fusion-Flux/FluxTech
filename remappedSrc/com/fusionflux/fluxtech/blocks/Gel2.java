...
public class Gel2 extends Block {
    ...
    public Gel2(AbstractBlock.Settings settings) {
        super(settings);
        ...
        if (entity.isOnGround()) {
            if (!entity.isSneaking()) {
                if (entity.getVelocity().y < 2)
                    entity.setVelocity(entity.getVelocity().add(0, 2.0D, 0));
            }
        }
    }
    ...
    protected boolean canConnect(WorldAccess world, BlockPos neighborPos) {
        return world.getBlockState(neighborPos).getBlock() == FluxTechBlocks.REPULSION_GEL;
    }
    ...
    private boolean canRunOnTop(BlockView world, BlockPos pos, BlockState floor) {
        return floor.isSideSolidFullSquare(world, pos, Direction.UP) && !floor.isOf(FluxTechBlocks.REPULSION_GEL);
    }
